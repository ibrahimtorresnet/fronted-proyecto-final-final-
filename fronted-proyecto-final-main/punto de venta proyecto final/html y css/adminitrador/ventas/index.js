...
// Agrega un controlador de eventos para el desplazamiento de los subpuntos
document.addEventListener('DOMContentLoaded', function() {
  // Selecciona el elemento con la clase 'mas' que contiene el enlace 'Productos'
  const productosElement = document.querySelector('.mas');
   
  // Selecciona todos los elementos con la clase 'sub_puntos'
  const subPuntosElements = document.querySelectorAll('.sub_puntos');

  // Agrega un controlador de eventos de clic al elemento 'Productos'
  productosElement.addEventListener('click', function(event) {
    // Previene la acción por defecto del enlace
    event.preventDefault();

    // Itera sobre cada elemento 'sub_puntos' y alterna su visibilidad
    subPuntosElements.forEach(subPunto => {
      subPunto.classList.toggle('hidden'); // Asume que tienes una clase 'hidden' que oculta elementos
    });
  });
});
